...
const addressSchema: Yup.Schema<AddressBaseProps> = Yup.object().shape({
...
const guardianSchema: Yup.Schema<NameBaseProps> = Yup.object().shape({
...
export const studentDetailsSchema: Yup.Schema<StudentDetailsProps> =
  Yup.object().shape({
    ...
    lastSchoolAttended: baseValidation(3, 255, true),
  });

const personalDetailsSchema: Yup.Schema<PersonalDetailsProps> =
  Yup.object().shape({
    firstName: baseValidation(3, 100, true),
    middleName: baseValidation(3, 100, true),
    lastName: baseValidation(3, 100, true),
    ...
    facebookLink: baseValidation(3, 255, true),
  });

const applicantSchema: Yup.Schema<ApplicantModelProps> = Yup.object().shape({
